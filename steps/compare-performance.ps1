...
try {
    if ($Publish) {
        ...
        Write-Host "(Re)creating gh-images branch..."
        git -C $RepoName update-ref -d refs/heads/gh-images # delete local gh-images if exists
        git -C $RepoName switch --orphan gh-images
